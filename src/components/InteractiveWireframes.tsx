...
const MAIL_MESSAGES = [
  {
    ...
    body: 'Hey! I found the same model at the hardware store on 5th — they have the replacement cartridge in stock. I can pick one up for you if you want. Also, the tutorial I mentioned is at plumber-help.web4.local/faucet-cartridge — the guy who made it has a 0.94 trust score in plumbing, so the instructions are solid.',
    trustBreakdown: { talent: 0.89, training: 0.84, temperament: 0.88 },
    history: '2 years active · 156 helpful actions · 0 disputes',
  },
  {
    ...
    body: 'Your answer to "How do I handle async errors in TypeScript?" was marked helpful by 12 community members. You earned 20 ATP. Your Talent score in "typescript" rose from 0.78 to 0.79. Keep contributing — you\'re 3 helpful answers away from the "Trusted Mentor" threshold.',
    trustBreakdown: { talent: 0.91, training: 0.93, temperament: 0.89 },
    history: 'Community account · 4,200 members · 12,000 helpful answers tracked',
  },
  {
    from: 'New Contact', trust: 0.43, cost: '8 ATP', subject: 'Freelance inquiry', time: '10:30 AM', color: 'text-amber-400',
    body: 'Hi, I saw your portfolio and I\'m interested in a logo design project. Budget is $500. Can we discuss?',
    trustBreakdown: { talent: 0.51, training: 0.38, temperament: 0.41 },
    history: '3 weeks active · 4 actions · no track record yet',
    warning: 'Low trust score — this person is new to Web4. The 8 ATP cost they paid to reach you shows genuine intent, but verify claims independently.',
  },
];

export function InteractiveMailWireframe() {
    ...
    <WireframeFrame title="Web4 Mail" url="mail.web4.local/inbox">
      <div className="overflow-hidden" style={{ background: 'rgba(17, 24, 39, 0.8)' }}>
        ...
        <div className="divide-y divide-gray-800">
          {MAIL_MESSAGES.map((msg, i) => (
            <div key={i}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs font-mono ${msg.color}`}>{msg.trust.toFixed(2)}</span>
                  </div>
              ...
              {expandedMsg === i && (
                <div className="px-4 pb-4 border-t border-gray-800/50 bg-gray-900/40 animate-in fade-in">
                  <p className="text-xs text-gray-300 py-3 leading-relaxed">{msg.body}</p>
                  {msg.warning && (
                    <div className="mb-3 px-3 py-2 rounded bg-amber-950/30 border border-amber-800/30 text-xs text-amber-300">
                      ⚠ {msg.warning}
                    </div>
                  )}
                  <div className="bg-gray-800/50 rounded-lg p-3 text-xs">
                    <div className="text-gray-500 mb-2 font-medium">Sender Trust Profile</div>
                    <div className="grid grid-cols-3 gap-3 mb-2">
                      {Object.entries(msg.trustBreakdown).map(([dim, val]) => (
                        <div key={dim}>
                          <div className="text-gray-500 capitalize mb-0.5">{dim}</div>
                          <div className="h-1.5 rounded-full bg-gray-700 overflow-hidden">
                            <div
                              className={`h-full rounded-full ${val >= 0.7 ? 'bg-emerald-400' : val >= 0.5 ? 'bg-sky-400' : 'bg-amber-400'}`}
                              style={{ width: `${val * 100}%` }}
                            />
                          </div>
                          <div className={`font-mono mt-0.5 ${val >= 0.7 ? 'text-emerald-400' : val >= 0.5 ? 'text-sky-400' : 'text-amber-400'}`}>
                            {val.toFixed(2)}
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="text-gray-600">{msg.history}</div>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="px-4 py-2 text-xs text-gray-600 border-t border-gray-800">
          <span className="text-sky-400/70">Click a message</span> to see the full content and sender&apos;s trust profile. 0 spam — messages cost energy to send.
        </div>
      </div>
    </WireframeFrame>
  );
...
const CANDIDATES = [
  {
    ...
    detail: {
      ...
      rate: '45 ATP/hour · high-trust discount applied (−30%)',
    },
  },
  {
            ...
            >
              ...
              {expandedCandidate === i && (
                <div className="border-t border-gray-700 p-3 bg-gray-900/40 text-xs space-y-2">
                  ...
                  <button className="mt-1 px-3 py-1.5 rounded bg-purple-600/30 border border-purple-500/40 text-purple-300 hover:bg-purple-600/50 transition-colors text-xs">
...
const REVIEWS = [
  {
    ...
    reviews: '43 restaurant reviews · 89% marked helpful',
  },
  {
    ...
    reviews: '12 restaurant reviews · 75% marked helpful',
  },
];
        ...
        <div className="p-4 space-y-3">
          {REVIEWS.map((r, i) => (
            <div key={i}>
              ...
              >
                <div className="w-7 h-7 rounded-full bg-gray-700 flex items-center justify-center text-xs text-gray-400 shrink-0 mt-0.5">{r.initials}</div>
                ...
                <span className="text-xs text-amber-400 shrink-0">{'★'.repeat(r.rating)}{'☆'.repeat(5 - r.rating)}</span>
              </div>
              {expandedReview === i && (
                <div className="ml-10 mt-2 p-3 bg-gray-800/40 rounded-lg text-xs">
                  ...
                  <div className="text-gray-500">{r.reviews}</div>
                </div>
              )}
            </div>
          ))}
...
export function InteractiveMarketWireframe() {
  ...
  const ESCROW_STEPS = [
    ...
    { label: 'You confirm receipt', detail: '350 ATP released to seller. Both trust scores updated. Done.', icon: '✅' },
  ];
  ...
  return (
    <WireframeFrame title="Web4 Market" url="market.web4.local/listing/used-camera">
      <div className="overflow-hidden" style={{ background: 'rgba(17, 24, 39, 0.8)' }}>
        ...
        <div className="px-4 py-2 text-xs text-gray-600 border-t border-gray-800">
          <span className="text-violet-400/70">{showEscrow ? 'Step through the escrow flow' : 'Click "Buy"'}</span> — no fake reviews, no paid badges. Every number traces back to real interactions.
        </div>
      </div>
    </WireframeFrame>
  );
